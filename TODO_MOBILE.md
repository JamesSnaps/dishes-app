...
- [ ] **Filter changes still round-trip.** The list now renders from local data, but `RecipeFilters` pushes a URL change and Next waits for the RSC payload before committing. Making filtering feel instant needs shallow URL updates (`history.replaceState`) when local data is available — the remaining half of this screen's win
